...
class BACCreditCardParser:
    ...
    FECHA_PAGO_PATTERN = re.compile(r"Fecha límite pago (?:mínimo|de contado):\s*(\d{1,2}-[A-Z]{3}-\d{2})")
    LIMITE_PATTERN = re.compile(r"Límite de crédito:\s*USD\s*([\d,]+\.?\d*)")
    DISPONIBLE_PATTERN = re.compile(r"Saldo disponible:\s*USD\s*([\d,]+\.?\d*)")
    IBAN_CRC_PATTERN = re.compile(r"Cuenta IBAN colones:\s*(CR\d{2}\s*\d{4}\s*\d{4}\s*\d{4}\s*\d{4}\s*\d{2})")
    IBAN_USD_PATTERN = re.compile(r"Cuenta IBAN dólares:\s*(CR\d{2}\s*\d{4}\s*\d{4}\s*\d{4}\s*\d{4}\s*\d{2})")
    ...
    def _extract_transactions(
        ...
        for line in lines:
            ...
            if "detalle de compras" in line_lower or "b) detalle" in line_lower:
                current_section = "compra"
                continue
            elif "detalle de intereses" in line_lower or "c) detalle de intereses" in line_lower:
                current_section = "interes"
                continue
            elif "detalle de otros cargos" in line_lower or "d) detalle" in line_lower:
                current_section = "cargo"
                continue
            elif "productos y servicios" in line_lower or "e) detalle" in line_lower:
                current_section = "seguro"
                continue
            elif "detalle de pago" in line_lower or "a) detalle" in line_lower:
            ...
            if any(skip in line for skip in [
                ...
                "Monto en colones", "colones", "dólares", "TARJETA DE CREDITO",
                "Total de compras", "Total de intereses",
            ]):
                continue
