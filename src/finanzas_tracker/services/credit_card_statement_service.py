...
class CreditCardStatementService:
    ...
    def _find_or_create_card(
        ...
        if existing:
            ...
            if statement.metadata.limite_credito_usd:
                existing.limite_credito = statement.metadata.limite_credito_usd * Decimal("500")  # Aprox USD→CRC
        ...
        card = Card(
            ...
            banco=BankName.BAC,
            marca=marca,
            limite_credito=statement.metadata.limite_credito_usd * Decimal("500") if statement.metadata.limite_credito_usd else None,
    ...
    def _create_billing_cycle(
        ...
        logger.info(f"📅 Nuevo ciclo creado: {fecha_corte} - Pago: ₡{statement.metadata.pago_contado_crc:,.2f}")
        return cycle
    ...
    def _create_transaction(
        ...
        import hashlib
        unique_str = f"{card.id}_{tx.referencia}_{tx.fecha.isoformat()}_{tx.concepto}_{tx.monto_crc}"
    ...
    def _parse_marca(self, marca_str: str) -> str:
        ...
        if "VISA" in marca_upper:
            return "VISA"
        elif "MASTER" in marca_upper:
            return "MASTERCARD"
        elif "AMEX" in marca_upper or "AMERICAN" in marca_upper:
            return "AMEX"
        else:
            return marca_str
