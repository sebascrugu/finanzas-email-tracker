...
class BankAccountStatementService:
    ...
    def _create_transaction(
        ...
        unique_str = f"bank_{tx.cuenta_iban}_{tx.referencia}_{tx.fecha.isoformat()}_{tx.concepto}_{tx.monto}"
        ...
        # Monto: para débitos es negativo en la BD (gasto), créditos positivo
        monto = tx.monto if tx.tipo == "credito" else -tx.monto
        ...
        transaction = Transaction(
            ...
            monto_crc=abs(tx.monto) if moneda == Currency.CRC else None,
