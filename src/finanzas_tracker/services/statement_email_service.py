...
if TYPE_CHECKING:
    from finanzas_tracker.services.credit_card_statement_service import ConsolidationResult
...
class StatementEmailService:
    ...
    def _is_statement_email(self, subject: str, sender: str) -> bool:
        ...
        has_keyword = any(
            ...
            for keyword in STATEMENT_SUBJECT_KEYWORDS
        )
    ...
    def _detect_statement_type(self, sender: str, subject: str) -> str:
        ...
        if sender_lower in [s.lower() for s in CREDIT_CARD_SENDERS]:
            return "credit_card"
        elif sender_lower in [s.lower() for s in BANK_ACCOUNT_SENDERS]:
            return "bank_account"
        
        # Detectar por palabras clave en asunto
        if "tarjeta" in subject_lower or "crédito" in subject_lower:
            return "credit_card"
        elif "cuenta bancaria" in subject_lower or "ahorro" in subject_lower:
            return "bank_account"
    ...
    def fetch_statement_emails(
        ...
        sender_filters = " or ".join([
            f"from/emailAddress/address eq '{s}'" 
            for s in STATEMENT_SENDERS
        ])
        filter_query = (
            ...
            for email in emails:
                ...
                for att in attachments:
                    if att.get("name", "").lower().endswith(".pdf"):
                        ...
                        tipo_label = "💳 Tarjeta" if statement_type == "credit_card" else "🏦 Cuenta"
