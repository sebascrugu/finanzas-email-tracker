...
class RecurringExpensePredictor:
    ...
    def predecir_gastos(
        ...
        predicciones.extend(
            self._detectar_gastos_periodicos(
                profile_id, dias_adelante, confianza_minima
            )
        )
    ...
    def generar_resumen_mensual(
        ...
        gastos_mes = [
            ...
            if inicio_mes <= p.fecha_estimada <= fin_mes
        ]
    ...
    def get_alertas_vencimiento(
        ...
        alertas = [
            p for p in predicciones
            if p.nivel_alerta in (AlertLevel.WARNING, AlertLevel.URGENT)
        ]
    ...
    def estimar_flujo_caja(
        ...
        """
        predicciones = self.predecir_gastos(
            profile_id, dias_adelante=dias, confianza_minima=50
        )
        ...
        for dia in range(dias + 1):
            ...
            gastos_dia = sum(
                p.monto_estimado for p in predicciones
                if p.fecha_estimada == fecha
            )
    ...
    def _determinar_alerta(
        ...
        elif dias_restantes <= 7 and es_grande:
    ...
    def _detectar_gastos_periodicos(
        ...
        for comercio, transacciones in por_comercio.items():
            ...
            for i in range(1, len(transacciones)):
                delta = (
                    transacciones[i].fecha_transaccion 
                    - transacciones[i-1].fecha_transaccion
                ).days
            ...
            prediccion = PredictedExpense(
                ...
                confianza=confianza,
                dias_restantes=dias_restantes,
                nivel_alerta=self._determinar_alerta(
                    dias_restantes, Decimal(str(monto_promedio))
                ),
